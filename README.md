...
- `pipeline_cluster_messages` — Message-cluster links with **context_block_id** (UUID) for traceability to Qdrant vectors
- `pipeline_topic_summaries` — Daily topic summaries with engagement metrics

**Note:** The `context_block_id` column stores real UUIDs generated by `contextBuilder.js`, enabling direct lookup of context blocks in Qdrant for RAG retrieval.
...
*Alternative considered:* We initially considered HDBSCAN clustering on embeddings, but that approach required a Python subprocess and produced inconsistent cluster counts depending on hyperparameters. TextTiling is deterministic, runs in pure JavaScript, and produces segments with clear boundaries that the LLM can then label reliably.
...
The pipeline runs entirely in Node.js with no Python dependencies.
...
│   │   ├── contextBuilder.js       # Context blocks with UUIDs
│   │   ├── embedder.js
│   │   ├── qdrantClient.js
│   │   ├── classifier.js
│   │   ├── storeResults.js         # Stores UUIDs to Supabase
│   │   ├── topicSummarizer.js      # LLM summaries per topic
│   │   ├── batchTracker.js
│   │   └── logger.js
...
│       ├── cleaning-cron/
│       ├── retention-cleanup/
│       └── pipeline-cron/   # Edge Function pipeline (with UUIDs)
...
### v6.9 — Context Block UUIDs & Dead Code Cleanup (Latest)
**Date:** March 29, 2026

**Bug Fixes:**
- Fixed `context_block_id` in `pipeline_cluster_messages` to store real UUIDs instead of "0"
- Fixed `messages_per_hour` calculation to use 1-minute minimum (was 1-hour) for accurate metrics

**Cleanup:**
- Removed all HDBSCAN/Python dependencies (now 100% Node.js)
- Deleted dead files: `debug_qdrant.js`, `_debug.js`, `test4-runner.js`, `clusterRunner.js`, `requirements.txt`
- Removed Python cache entries from `.gitignore`

**Changes:**
- `storeResults.js` now builds context blocks before storing, mapping messages to UUIDs
- Supabase Edge Function (`pipeline-cron/index.ts`) now includes `buildContextBlocks()`
- Updated documentation to reflect UUID-based traceability to Qdrant

**Deployment:** No database migrations required — backward compatible

### v6.8 — Multi-User Thread Support
- Sub-issue creation for multiple users in same thread
- Thread brief system with pinned embeds
- Ball-holder detection for smart reminders

### v6.0 — RAG Support Bot
- 4-layer RAG pipeline (Intent → Context → Rewrite → Respond)
- Vector search + reranking with Cloudflare AI
- Escalation system with cooldown logic
